...
def make_table_one(final_df: pd.DataFrame,
                   patient_cohort: pd.DataFrame,
                   id_col: str = "encounter_block") -> pd.DataFrame:
    """
    Summarize final_df (encounters) and patient_cohort (patients)
    into a long DataFrame with columns: Variable, Value.
    
    Returns one row per measure, with 'Overall' as the column header.
    """
    rows = []
    # — 0. Years —
    # rows.append(("Years (Start-End)", 
    #              f"{final_df['admission_dttm'].dt.year.min()} - {final_df['admission_dttm'].dt.year.max()}"))
    ...
    df_pat = final_df[[id_col, "patient_id"]].drop_duplicates("patient_id")
    N_pat = df_pat["patient_id"].nunique()
    rows.append(("N: unique patients",                f"{N_pat}"))
    
    # — 2. Demographics (patient‐level) —
    # merge in the demographics and compute age at first ICU
    demo = (
        df_pat
        .merge(final_df[[id_col, "age_at_admission"]].drop_duplicates(id_col),
               on=id_col)
        .merge(patient_cohort, on="patient_id", how="left")
    )
    
    # helper for n(%)
    def cat_n_pct(df, col, title):
        vc = df[col].value_counts(dropna=False)
        total = len(df)
        for lvl, cnt in vc.items():
            pct = cnt/total*100
            rows.append((f"{title}: {lvl}", f"{cnt} ({pct:.1f}%)"))
    
    cat_n_pct(demo, "race_new",     "Race n (%)")
    cat_n_pct(demo, "sex_category",      "Sex n (%)")
    cat_n_pct(demo, "ethnicity_category","Ethnicity n (%)")
    
    # — 3. Mortality (encounter‐level) —
    mort_n   = (final_df["is_dead"] == 1).sum()
    mort_pct = mort_n/ N_enc * 100
    rows.append(("Hospital mortality n (%)", f"{mort_n} ({mort_pct:.1f}%)"))
    
    # — 4. SOFA (first 24 h) —
    # Total SOFA
    sofa = final_df["sofa_total"].dropna()
    rows.append(("SOFA-97 TOTAL, median [Q1,Q3]",
                 f"{sofa.median():.1f} [{sofa.quantile(0.25):.1f},{sofa.quantile(0.75):.1f}]"))
    
    # SOFA components
    for component in ['sofa_cv_97', 'sofa_coag', 'sofa_liver', 'sofa_resp_pf',
                     'sofa_resp_pf_imp', 'sofa_resp', 'sofa_cns', 'sofa_renal']:
        sofa_comp = final_df[component].dropna()
        # Clean up component name for display
        comp_name = component.replace('sofa_', '').replace('_', ' ').upper()
        rows.append((f"SOFA {comp_name}, median [Q1,Q3]",
                    f"{sofa_comp.median():.1f} [{sofa_comp.quantile(0.25):.1f},{sofa_comp.quantile(0.75):.1f}]"))
    
    # — 5. First location of admission —
    cat_n_pct(final_df, "first_admission_location",     "Admission Location n (%)")
    
    # — 6. Total ICU days & Hospital days —
    icu  = final_df["first_icu_los_days"].dropna()
    hosp = final_df["hospital_length_of_stay_days"].dropna()
    rows.append(("Total ICU days, median [Q1,Q3]",
                 f"{icu.median():.1f} [{icu.quantile(0.25):.1f},{icu.quantile(0.75):.1f}]"))
    rows.append(("Total hospital days, median [Q1,Q3]",
                 f"{hosp.median():.1f} [{hosp.quantile(0.25):.1f},{hosp.quantile(0.75):.1f}]"))
    
    # — 7. Vasopressor encounters & dosing —
    vp_n   = final_df["on_pressor"].sum()
    vp_pct = vp_n/ N_enc * 100
    rows.append(("Vasopressor encounters n (%)", f"{vp_n} ({vp_pct:.1f}%)"))

    # how many distinct pressors per encounter → take mean±sd
    npart = final_df["n_pressors"].fillna(0)
    ...
    imv_n   = final_df["on_vent"].sum()
    imv_pct = imv_n/ N_enc * 100
    rows.append(("IMV encounters n (%)", f"{imv_n} ({imv_pct:.1f}%)"))

    cat_n_pct(imv_subset, "first_location_imv",     "First location of intubation n (%)")
    cat_n_pct(imv_subset, "initial_mode_category",     "Initial mode for IMV n (%)")

    # ventilator settings
    for v in ["fio2_set","peep_set", "resp_rate_set", "tidal_volume_set"]:
        median = imv_subset[f"{v}_median"].dropna()
        q1     = imv_subset[f"{v}_q1"].dropna()
        q3     = imv_subset[f"{v}_q3"].dropna()
        rows.append((f"{v}, median [Q1,Q3]",
                     f"{median.median():.2f} [{q1.quantile(0.25):.2f},{q3.quantile(0.75):.2f}]"))
        
    # — assemble into DataFrame —
    table_one = pd.DataFrame(rows, columns=["Variable","Overall"])
    return table_one