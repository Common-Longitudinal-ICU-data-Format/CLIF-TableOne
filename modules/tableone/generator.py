...
def main(memory_monitor=None, cohort_mode='critical_illness') -> bool:
    ...
    # Sub-strata of advanced_resp: advanced respiratory support recipients with
    # vs. without any ICU stay. Built after both high_support_enc and icu_enc
    # are finalized (and the is_procedural_ld_only zero-out at L1728 has already
    # run in critical-illness mode), so the two flags partition the parent
    # advanced_resp cohort exactly.
    ...
    final_cohort_merge_cols = ['encounter_block', 'high_support_enc',
    ...
    for _strat_by_year_iter in (range(1) if cohort_mode != 'ward' else range(0)):
        ...
        for stratum_name, col in encounter_type_strata.items():
            ...
            # the slash so nested keys like 'advanced_resp/icu' resolve to
            # advanced_resp/icu/tableone/.
